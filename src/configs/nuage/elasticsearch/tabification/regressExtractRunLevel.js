...
export default function regressExtractRunLevel(response, query = {}) {
    let table;
...
function populateRunLevel(table){
    var regex = /(.* -runLevel) (\w+) (.*)/g;
    table.forEach(item => {
        item.runLevel = item.args.replace(regex,"$2");
    })
    return table;
}
