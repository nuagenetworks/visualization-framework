...
function selectRow(state, id, data, matchedRows, parentQuery, parentPathname) {
    ...
    return state.setIn([ActionKeyStore.SELECTED_ROW, id, ActionKeyStore.SELECTED_ROW_DATA], data)
                .setIn([ActionKeyStore.SELECTED_ROW, id, ActionKeyStore.SELECTED_MATCHED_ROW_DATA], matchedRows)
...
function VFSReducer(state = initialState, action) {
    ...
    switch (action.type) {
        case ActionTypes.ACTION_SELECT_ROW:
            return selectRow(state, action.id, action.data, action.matchedRows, action.parentQuery, action.parentPath);
